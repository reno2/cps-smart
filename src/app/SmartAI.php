...
class SmartAI {
    ...
    public function getSmartTitle(): array
    {
        ...
        return ['smart_title' => $response_data['choices'][0]['message']['content']];
    }
    ...
    private function prepareBody(): array
    {
        return [
            ...
            'messages' => [
                ...
                Крайний срок: "22.08.2024".'
                ],
                [
                    'role' => 'assistant',
                    'content' => 'КР Томск: Реализация функционала выгрузки списка подробного чек-листа
             загруженных отчетов с учетом фильтров и сортировок в xlsx-файл в административном модуле до 22.08.2024.'
                ],
                [
                    'role' => 'user',
                    'content' => '
                        ...
                        Крайний срок: "'.$this->payload["deadline"].'".
                    '
                ]
            ],
            'temperature' => 0.8
